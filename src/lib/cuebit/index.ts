...
class Cuebit {
    ...
    public process(data: Uint8ClampedArray): CuebitResult {
        ...
        const lowRed  = new cv.Mat(this.hsv.rows, this.hsv.cols, this.hsv.type(), [0, 120, 70, 0]);
        const highRed = new cv.Mat(this.hsv.rows, this.hsv.cols, this.hsv.type(), [10, 255, 255, 0]);
