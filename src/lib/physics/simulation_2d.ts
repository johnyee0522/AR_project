...
export const DEFAULT_SIMULATION_2D_TUNING: Simulation2DTuning = {
	...
	spinInputReferenceMm: BALL_DIAMETER_MM,
	// 기준값보다 큰 스핀 입력을 허용하는 최대 비율
	maxSpinRatio: 2,
	// 좌/우 스핀 효과를 전체적으로 키우거나 줄이는 배율
	sideSpinStrength: 2.5,
	// 상/하 스핀 효과를 전체적으로 키우거나 줄이는 배율
	topSpinStrength: 1.5,
...
// 내부 물리 좌표는 2.84m x 1.42m 당구대를 meter 단위로 사용
interface BallState {
	id: string;
	position: Vec2;
	velocity: Vec2;
...
export class Simulation2D {
	...
	public predict(
		...
		// 예측 중에는 상태를 바꾸고, 끝나면 원래 상태로 복구한다.
		...
		// 렌더러는 waypoint 사이를 직선으로 이어 궤적을 그린다.
		// 실제 이동거리와 궤적 길이가 맞도록 마지막 정지 위치를 반드시 포함한다.
	...
	private stepSimulation(
		...
		// 먼저 모든 공을 이동시키고, 이동 중 쿠션 충돌은 연속 충돌 방식으로 처리한다.
		...
		// 공-공 충돌은 이전 위치와 현재 위치 사이의 이동 경로를 검사해 빠른 공의 터널링을 줄인다.
	...
	private advanceBall(
		...
		// 한 프레임 안에서도 쿠션에 맞고 다시 움직일 수 있다.
		// 충돌 시점까지의 시간만 먼저 사용하고, 반사 후 남은 시간만큼 계속 진행한다.
		while (remainingTime > 1e-6 && collisionCount <= maxCollisionsPerStep) {
			...
			// 이벤트 중복 제거와 실제 충돌 발생 여부를 분리한다.
			// 같은 쿠션 접촉이 재기록되지 않아도 남은 시간 이동은 계속 처리해야 한다.
	...
	private advanceBallSegment(
		...
		// 구간 안에 쿠션 충돌이 없으면 전체 시간 동안 감속 이동을 적용한다.
		if (!impact) {
			ball.position = {
		...
		// 구간 안에 쿠션 충돌이 있으면 정확한 접촉 지점까지 이동한다.
		// 그동안 줄어든 속도를 반영한 뒤 쿠션에서 반사한다.
	...
	private timeForTravel(
		...
		// travelDistance 역계산으로 특정 거리까지 도달하는 시간을 구한다.
	...
	private reflectHorizontalCushion(
		...
		const tangentVelocity = ball.velocity.x;
		const tangentSpeed = Math.abs(tangentVelocity);
	...
	private resolveBallCollisions(
		...
		for (let i = 0; i < balls.length; i++) {
			for (let j = i + 1; j < balls.length; j++) {
				...
				// 이전 위치부터 현재 위치까지 두 공의 이동 경로를 비교해 첫 충돌 시점을 찾는다.
				...
				// 법선 방향 속도만 반발계수로 교환한다.
				// 같은 질량의 공끼리 부딪힌다고 보고 충돌 방향으로 impulse를 적용한다.
	...
	private findBallImpact(
		...
		// 두 프레임 사이 빠른 수구가 목적구를 지나쳐버리는 상황을 막기 위해 t 범위 [0, 1]에서 충돌 시점을 푼다.
		...
		// 부동소수점 오차로 첫 접촉 시점을 놓친 경우에만 현재 겹침을 fallback으로 사용한다.
		if (currentDist > 1e-8 && currentDist <= minDist) {
			return { time: 1, normal: scale(currentDelta, 1 / currentDist) };
		}
		return null;
	...
	private applyCueSpinAfterBallCollision(
		...
		// 충돌 후 스핀 보정은 수구의 스핀에만 적용한다.
		// 목적구가 임의의 접선 이동으로 다시 수구에 영향을 주지 않도록 제한한다.
		...
		// 상/하 스핀은 간단한 근사 모델로 처리한다.
		// 정면에 가까운 충돌일수록 상/하 스핀 영향이 더 크게 남는다.
	...
	private settleStoppedBalls(): void {
		for (const ball of this.balls.values()) {
			...
			// 제자리 회전은 이후 경로를 바꾸지 않으므로 예측 루프 종료를 위해 제거한다.
			ball.sideSpin = 0;
			ball.topSpin = 0;
		}
	}
	...
	private normalizeTipOffset(offsetMm: number): number {
		// UI 스핀 입력을 기준값 대비 비율로 바꾸고 과도한 입력만 제한한다.
