...
export class Simulation {
    ...
    private createWall(x: number, y: number, hx: number, hy: number) {
        ...
        const colliderDesc = this.rapier.ColliderDesc.cuboid(hx, hy).setRestitution(0.85).setFriction(0.2);
    ...
    public createBall(pxX: number, pxY: number) {
        ...
        const body = this.world.createRigidBody(bodyDesc);
        const colliderDesc = this.rapier.ColliderDesc.ball(this.BALL_RADIUS_M).setRestitution(0.92).setFriction(0.1).setDensity(1.0);
        this.world.createCollider(colliderDesc, body);
    ...
    public toPixelVector(metersX: number, metersY: number) {
        return { x: metersX * this.ppm, y: metersY * this.ppm };
    }

    public predictTrajectory(cueBall: RAPIER.RigidBody, allBalls: RAPIER.RigidBody[], angleDeg: number, power: number, maxSteps: number = 180) {
        const backupStates = allBalls.map(ball => ({
            translation: ball.translation(), rotation: ball.rotation(),
            linvel: ball.linvel(), angvel: ball.angvel()
        }));

        const angleRad = (angleDeg * Math.PI) / 180;
        cueBall.applyImpulse({ x: Math.cos(angleRad) * power, y: Math.sin(angleRad) * power }, true);
        ...
        for (let i = 0; i < maxSteps; i++) {
            ...
            if ((vel.x * vel.x + vel.y * vel.y) < 0.001) break;
        }
